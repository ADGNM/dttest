...
$(function() {
	...
	var todaytimeminutes;
	var todaytimehours;
	if (todaynow.getMinutes() < 10) {
		todaytimeminutes = "0" + todaynow.getMinutes()
	} else {
		todaytimeminutes = todaynow.getMinutes()
	}
	if (todaynow.getHours() < 10) {
		todaytimehours = "0" + todaynow.getHours()
	} else {
		todaytimehours = todaynow.getHours()
	}
	var todaytime = todaytimehours + ":" + todaytimeminutes;
	if (todaytime >= "08:40" && todaytime <= "08:50" || todaytime >= "09:30" && todaytime <= "09:40" || todaytime >= "11:00" && todaytime <= "11:10" || todaytime >= "14:40" && todaytime <= "14:50" || todaytime >= "15:00" && todaytime <= "15:10" || todaytime >= "16:20" && todaytime <= "16:40") {
		$("#fwl").parent().before('<div id="xhwllycshow" style="display:none"><iframe width="0" height="0" src="http://www.xinhuanet.com/dzxw2/iframe_henant.htm" style="display:none;"></iframe></div>');
		$("#xhwllycshow").attr("id", 'xhwllychide');
		$("#xhwllychide").remove();
	}
});
		...
		return 0 == c.event.indexOf("scroll") && a(c.container).bind(c.event, function(b) {
			...
			b.loaded = !1, a(b).one("appear", function() {
				this.loaded || a("<img />").bind("load", function() {
					a(b).hide().attr("src", a(b).data("original"))[c.effect](c.effectspeed), b.loaded = !0
